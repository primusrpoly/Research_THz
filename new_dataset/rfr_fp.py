...
print(f'RFR A20: {a_20}')


### INPUT PHASE NOISE ###
phase_noise_value = 25

# ### Desired BER target ###
# desired_ber_target = 0


print(f"Finding optimal parameters for {phase_noise_value} GHz using RFR...")
...
#SR -> low as possible
def penalty_symbol_rate(symbol_rate, min_rate=1e8):
    return (symbol_rate - min_rate) / min_rate

#SNR -> low as possible
def penalty_snr(snr, min_snr=10):
...
def combined_objective(params):
    ...
    weight_penalty = 0.001 #penalty weight
    # print(ber)
    # print(penalty)
    # print(weight_ber * ber)
    # print(weight_penalty * penalty)
    # print(weight_ber * ber + weight_penalty * penalty)
...
def find_optimal_parameters(phase_noise_value):
    ...
    result = differential_evolution(de_objective, bounds[1:], strategy='best1bin', maxiter=10, popsize=10, tol=1e-4)
