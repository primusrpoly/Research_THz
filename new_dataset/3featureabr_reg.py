...
# df_train['CBER'] = y_train  # Add the target variable for completeness
# df_test = X_test.copy()
# df_test['CBER'] = y_test
...
# # Export the train and test data to Excel in separate sheets
...
for depth_idx, rate in enumerate(l_rate):
    for tree_idx, n_trees in enumerate(trees):
        abr = AdaBoostRegressor(estimator=DecisionTreeRegressor(max_depth=n_trees,random_state=17, criterion='squared_error'),
                                n_estimators=5, learning_rate=rate, random_state=17)
...
with pd.ExcelWriter(file_path, mode='a', engine='openpyxl') as writer:
    df_metrics.to_excel(writer, sheet_name='ABR_SMAPE_d12', index_label='Depth')
    df_metricsA20.to_excel(writer, sheet_name='ABR_A20_d12', index_label='Depth')
    df_metrics_MAE.to_excel(writer, sheet_name='ABR_MAE_d12', index_label='Depth')