...
%% Parameters
symbol_rate = [1e6, 10e6, 30e6, 100e6, 300e6, 1e9, 10e9, 30e9]; %[1e6, 10e6, 30e6, 100e6, 300e6, 1e9, 10e9, 30e9]; % [Hz]
phase_noise = 0:5:30; %0:5:30; % -70 dBc/Hz + phase_noise, higher the value phase noise is higher
M = 4; %[4, 8, 16, 32, 64, 128, 256, 512, 1024]; % Modulation order
pilot_length = [2, 8, 16, 32, 64]; % length of th pilot [2, 8, 16, 32, 64];
symbols_between_pilot = [16, 64, 128, 256, 512, 1024]; % number of symbols between the pilots [16, 64, 128, 256, 512, 1024];
SNR_Value = 0:5:50; %0:5:50; % SNR considering AWGN noise
