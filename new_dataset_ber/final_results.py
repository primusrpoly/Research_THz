...
Accuracy_Values = cross_val_score(gbr, X, y, cv = cv_scores, scoring = custom_Scoring)
...
Accuracy_Values3 = cross_val_score(gbr,X ,y,\
...
CV = RepeatedKFold(n_splits = 5, n_repeats=3, random_state = 17)
Accuracy_Values5 = cross_val_score(gbr,X ,y,\
...
with pd.ExcelWriter(file_path, mode='a', engine='openpyxl') as writer:
    df_metrics_SMAPE.to_excel(writer, sheet_name='GBR_Final_SMAPE6', index=False, startrow=0, startcol=0)
    df_metrics_A20.to_excel(writer, sheet_name='GBR_Final_A206', index=False, startrow=0, startcol=0)
    df_metrics_MAE.to_excel(writer, sheet_name='GBR_Final_MAE6', index=False, startrow=0, startcol=0)