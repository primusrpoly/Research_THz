...
for neighbor in range(1,16):
    ...
    Accuracy_Values = cross_val_score(knn, X_train, y_train,\
                                      cv = cv_scores, scoring = custom_Scoring)
    trial += 1
...
df_metrics_smape = pd.DataFrame(smape_values, index=range(1, 16), columns=range(1, 16))   
...
with pd.ExcelWriter(file_path, mode='a', engine='openpyxl') as writer:
    df_metrics_smape.to_excel(writer, sheet_name='KNN_SMAPE', index=False, startrow=0, startcol=0)
