...
gbr = GradientBoostingRegressor(max_depth= 3, random_state=17, n_estimators= 50, learning_rate= 0.1)
...
#A20
df_metrics = pd.DataFrame(Accuracy_Values3, index=range(1, 16), columns=range(1, 2))   
    # 
file_path = "C:/spydertest/csv/CumulRot.xlsx"

# #Export the DataFrame to an Excel file on a specific sheet
with pd.ExcelWriter(file_path, mode='a', engine='openpyxl') as writer:
    df_metrics.to_excel(writer, sheet_name='GBra20final2', index=False, startrow=0, startcol=0)